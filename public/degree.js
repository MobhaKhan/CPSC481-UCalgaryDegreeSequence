...
const statusBadge = (token) => {
  ...
  if (normalized.includes('incomplete')) {
    return { label: 'Incomplete', icon: '⊙', variant: 'year-panel__badge--warning' }
  }
  if (normalized.includes('complete')) {
    return { label: 'Complete', icon: '✔', variant: 'year-panel__badge--success' }
  }
  if (normalized.includes('issue')) {
    return { label: 'Issue Found', icon: '⚠', variant: 'year-panel__badge--issue' }
  }
  if (normalized.includes('planning')) {
    return { label: 'Planning', icon: 'ℹ', variant: 'year-panel__badge--info' }
  }
  if (normalized.includes('track') || normalized.includes('progress')) {
    return {
      label: 'On Track',
      icon: '⊙',
      variant: 'year-panel__badge--muted'
  ...
  return `<div class="course-box-grid">
    ${courses
      .map((course) => {
        const catalog = getCatalogInfo(course.code)
        const tooltipLines = []
        if (catalog.prerequisites?.length) {
          tooltipLines.push(`Prerequisite(s): ${catalog.prerequisites.join('; ')}`)
        }
        if (catalog.antirequisites?.length) {
          tooltipLines.push(`Antirequisite(s): ${catalog.antirequisites.join('; ')}`)
        }
...
const renderPlanDetails = (plan) => {
  ...
  const termHtml = seasons
    .map((term) => {
      ...
      return `
        <div class="plan-panel__term">
          <h4>${termLabels[term]}</h4>
          <ul class="plan-panel__course-list">
            ${courses
              .map((course) => `<li class="plan-panel__course-chip">${escapeHtml(course)}</li>`)
              .join('')}
          </ul>
        </div>`
    })
  ...
  const remainingHtml =
    plan.remainingCourses?.length > 0
      ? plan.remainingCourses
          .map((course) => `<span class="chip chip-soft">${escapeHtml(course)}</span>`)
          .join('')
      : '<p class="empty-state">All tracked courses scheduled</p>'
  ...
  return `
    <aside class="plan-panel">
      <header class="plan-panel__header">
        <p>Plan Details</p>
        <span>${escapeHtml(plan.year)}</span>
        <button type="button" class="plan-panel__close">×</button>
      </header>
      <div class="plan-panel__terms">
        ${termHtml}
      </div>
...
const attachEvents = () => {
  // Switch between plans when tabs are clicked
  document.querySelectorAll('.plan-tab').forEach((tab) => {
    tab.addEventListener('click', () => {
      state.activePlanId = tab.dataset.planId
      render()
  ...
  // Close button inside plan panel
  const closeBtn = document.querySelector('.plan-panel__close')
  if (closeBtn) {
